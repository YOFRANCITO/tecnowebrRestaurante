...
const form = useForm({
    ...
    tasa_interes_diario: props.plan.tasa_interes_diario,
    plazo_dias: props.plan.plazo_dias,
});

function submit() {
    form.put(route('restaurant.planes.update', props.plan.id));
}
</script>

<template>
    <AppLayout>
        <Head title="Editar Plan de Crédito - Restaurante" />
            ...
            <div class="flex-1">
                <div class="mb-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h1 class="text-3xl font-bold" style="color: var(--color-text-primary)">Editar Plan de Crédito</h1>
                            <p class="text-sm mt-1" style="color: var(--color-text-secondary)">
                                Modifica los datos del plan {{ plan.nombre }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="card rounded-lg shadow-sm p-6 max-w-2xl">
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="flex items-center gap-4 pt-4">
                            ...
                            >
                                {{ form.processing ? 'Guardando...' : 'Actualizar Plan' }}
                            </button>
                            <Link
